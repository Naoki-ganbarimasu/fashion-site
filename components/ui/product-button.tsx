    ...
    return (
      <Button
        ...
        className={cn(
          ...
          isActive && variant === "wishlist" && "bg-pink-100 text-pink-600 hover:bg-pink-200 hover:text-pink-700",
          className,
        )}
        {...props}
      >
        {variant === "cart" && <ShoppingCart className="h-5 w-5" />}
        {variant === "wishlist" && <Heart className={cn("h-5 w-5", isActive && "fill-current")} />}
